...
```
결과 : milk-milk, yogurt-yogurt 있는 행을 걸러내는 데 실패
...
```
우유와 요거트를 한 장바구니에 담아놨을 때 셀프조인을 하면, milk-yogurt, yogurt-milk 조합이 출력\
즉, 조합이 순서만 바뀐 채로 중복되어서 출력되기 때문에, milk-yogurt or yogurt-milk 중 하나의 조합을 조건으로 지정
...
FROM BOOK_SALES A
JOIN BOOK B JOIN AUTHOR C ON A.BOOK_ID = B.BOOK_ID AND B.AUTHOR_ID = C.AUTHOR_ID
...
ORDER BY C.AUTHOR_ID, CATEGORY DESC
```
2nd 쿼리의 FROM절에서 서브쿼리를 넣는 방법이 잘못됐다고 생각하여, 1st 쿼리의 `SUM(SALES X PRICE)`만 수정\
...
```
ORDER BY와 LIMIT을 사용해서 리뷰를 가장 많이 남긴 회원을 찾으려 함\
...
ORDER BY REVIEW_DATE, REVIEW_TEXT
```
ORDER BY절의 집계함수와 SELECT절의 집계함수 통일\
...
결과 : 그룹핑때문에 2021년도에 가입한 회원들(A.USER_ID)이 COUNT에 잡히지 않고, 서브쿼리절와 서브쿼리 밖의 쿼리가 다를 게 없음
...
1. `FROM` : 회원가입을 하고 구매하지 않은 회원이 있을 것이라고 판단하여, `USER_INFO`와 `ONLINE_SALE`테이블을 LEFT JOIN 했다.
2. `WHERE` : 2021년에 가입한 회원 데이터를 필터링한다. 그리고, LEFT JOIN을 하여 값이 비어있는 `ONLINE_SALE` 테이블의 NULL값을 처리한다.
3. `GROUP BY` : 판매 년, 월 별로 그룹화한다.
4. `SELECT` : 판매 년, 월과 판매 년, 월 별 구매 회원 수를 출력한다. 그리고, (2021년 가입한 회원 중 구매 회원 수 / 2021년 가입한 전체 회원 수) 비율을 출력해야 하는데, 분자는 년, 월 별로 변동하는 값이고 분모는 고정값이다. 분모에 가입 회원 정보를 담은 테이블에서 가입일이 2021년인 데이터를 필터링하고, 회원 수를 출력하여 비율을 계산한다.
